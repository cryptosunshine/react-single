...
export const decodeData = (
  ...
  try {
    ...
    if (dataContent && dataContent.length === 0 && event && event.length > 0 && customUI) {
      let eventContent: any = [];
      let eResultArr:any = [];
      event.forEach((e: EventList, i: number) => {
      ...
      const uniqueArray = eResultArr.reduce((acc:EventList, current:EventList) => {
        const found = acc.find((item:EventList) => (item['address'] === current['address'] && ['ERC721_Mint','ERC721_Burn', 'ERC721_Transfer', 'ERC1155_Mint', 'ERC1155_Burn', 'ERC1155_Transfer'].includes(item['type'])));
        if (found) {
          // 如果找到了重复项，累加value
